...
public static class ApiKeyExtensions
{
  ...
  public static IEnumerable<ApiKey> Name(this IEnumerable<ApiKey> keys, string name) => keys.Where(key => (key?.AppName is not null && key.AppName.ToLower().StartsWith(name.ToLower())) || (key?.Name is not null && key.Name.ToLower().StartsWith(name.ToLower())));
}