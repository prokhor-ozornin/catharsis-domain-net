...
public static class NewsExtensions
{
  ...
  public static IEnumerable<News> Name(this IEnumerable<News> news, string name) => news.Where(news => news?.Name is not null && news.Name.ToLower().StartsWith(name.ToLower()));

  public static IQueryable<News> Tag(this IQueryable<News> news, Tag tag) => news.Where(news => news.Tags.Contains(tag));

  public static IEnumerable<News> Tag(this IEnumerable<News> news, Tag tag) => news.Where(news => news is not null && news.Tags.Contains(tag));
}