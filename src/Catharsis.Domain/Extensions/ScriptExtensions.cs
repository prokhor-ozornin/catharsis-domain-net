...
public static class ScriptExtensions
{
  ...
  public static IEnumerable<Script> Executed(this IEnumerable<Script> scripts, bool? executed) => scripts.Where(script => script is not null && script.Executed == executed);
  ...
  public static IEnumerable<Script> Name(this IEnumerable<Script> scripts, string name) => scripts.Where(script => script?.Name is not null && script.Name.ToLower().StartsWith(name.ToLower()));
  ...
  public static IEnumerable<Script> Duration(this IEnumerable<Script> scripts, long? from = null, long? to = null)
  {
    if (from is not null)
    {
      scripts = scripts.Where(script => script is not null && script.Duration >= from.Value);
    }

    if (to is not null)
    {
      scripts = scripts.Where(script => script is not null && script.Duration <= to.Value);
    }

    return scripts.Where(script => script is not null);
  }
}