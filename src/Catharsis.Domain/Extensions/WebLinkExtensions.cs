...
public static class WebLinkExtensions
{
  ...
  public static IEnumerable<WebLink> Name(this IEnumerable<WebLink> addresses, string name) => addresses.Where(link => link?.Name is not null && link.Name.ToLower().StartsWith(name.ToLower()));
}